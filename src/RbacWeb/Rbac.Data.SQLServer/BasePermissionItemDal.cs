...
{
    public class BasePermissionItemDal : BaseDAL<BasePermissionItem>, IBasePermissionItemDal
    {
        /// <summary>
        /// 创建公共方法
        /// </summary>
        ...
        protected override string CreateSQL(string where, string orderBy)
        {
            ...
            return string.Format(@"select * FROM (select m.*,m2.Name as ParentName,m2.SortCode AS ParentSortCode,a.amount,
                                            ...
                                            END AS isleaf FROM Base_PermissionItem as m
                                    LEFT JOIN Base_PermissionItem as m2
                                    ON m.ParentId=m2.Id
                                    LEFT JOIN (SELECT
                                                COUNT(1) AS amount,
                                                ParentId
